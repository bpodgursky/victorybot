 ...
 * 2-octet data representation is handled by the <code>Connection</code> class.
 * 
...
public class Connection {
	...
	/*
	 * XXX - Right now, I put us in an error state when an EM has been received.
	 * Look into this. private final static int EM_SENT = IDLE | 0x00;
	 */
	 ...
	 * @throws IOException
	 *             if data could not be sent to the server (Possibly because
	 *             there is no answer at the specified address)
	 * @throws DisconnectedException
	 *             if disconnected during the handshake process.
	 */
	public Connection(InetAddress ip, int port,
			DiplomaticMessageListener listener) throws IOException,
	...
	/**
	 * Disconnects from the AI server. Sends a <code>Final Message</code> to the
	 * server, then sets the state to "disconnected".
	 * 
	 * @throws IOException
	 ...
	 *             parser. Note that if all tokens are deemed to be ok, no
	 *             exception will be thrown even if the server labels a token as
	 *             unknown. In this case, an <code>Error Message</code> will be
	 *             received from the server instead.
	 * @throws DisconnectedException
	 *             if we're already disconnected from the server when attempting
	 *             to send this message.
	 */
	public void send(String[] message) throws UnknownTokenException,
			DisconnectedException {
	...
	/**
	 * Sends the message to the server. This method wraps the sending of the
	 * message to the server in a state transition.
	 * 
	 * @param preState
	 *            The state to set the connection into while sending the
	 *            message.
	 * @param message
	 *            The message to send.
	 * @param postState
	 *            The state to set the connection into if the message was sent
	 *            in good order.
	 */
	 ...
	 * 
	 * @return A well-formatted <code>Initial Message</code> with type=0,
	 *         version=1, magic number=0xDA,0x10
	 */
	byte[] createInitialMessage() {
		return new byte[] { (byte) 0x00, // Msg type
				(byte) 0x00, // pad
	...
	byte[] createDiplomaticMessage(byte[] data) {
		...
		System.arraycopy(data, 0, bytes, 4, data.length); // Language Message
		return bytes;
	}
	...
	byte[] createErrorMessage(byte code) {
		return new byte[] { (byte) 0x04, // Msg type
				(byte) 0x00, // pad
	...
	byte[] createFinalMessage() {
		return new byte[] { (byte) 0x03, // Msg type
				(byte) 0x00, // pad
	...
	/**
	 * Handles an incoming message. This method only delegates to the relevant
	...
	/**
	 * Handles unknown message types. This would only occur when the server
	 * sends badly formatted messages, or the well-formatted message is parsed
	 * badly by the receiving side.
	 * 
	 * @param type
	 *            The supposed type of the message
	 * @param message
	 *            The body of the message
	 */
	...
	/**
	 * Handles a <code>Representation Message</code>. Any tokens encoded in the
	 * message body will be added to the Token singleton. Upon correct decoding,
	 * the state will be set to <code>RM_RECEIVED</code>
	 * 
	 * @param message
	 *            The body of the message.
	 */
	void handleRepresentationMessage(byte[] message) {
		for (int i = 0; i < message.length; i += 6) {
			...
			byte[] token = new byte[] { message[i + 2], message[i + 3],
					message[i + 4] };
	...
	/**
	 * Handles a <code>Diplomatic Message</code>. The message will be converted
	 * to a string array of tokens, and passed on to our listener. Upon
	 * reception by our listener, the state will be set to
	 * <code>DM_RECEIVED</code>. <br>
	 * If the message contains a token that is unknown to us, the token
	 * conversion will fail. An <code>Error Message</code> will be sent to the
	 * server, and the state will be set to <code>EM_SENT</code>.
	 * 
	 * @param message
	 *            The body of the message. This body will be decoded into a
	 *            <code>String[]</code> which is sent to the listener.
	 */
	...
	/**
	 * Handles a <code>Final Message</code>. The state will be set to
	 * <code>FM_RECEIVED</code>.
	 */
	...
	/**
	 * Handles an <code>Error Message</code>. A textual description of the error
	 * code will be printed to <code>System.out</code>, after which the state
	 * will be set to <code>EM_RECEIVED</code>.
	 * 
	 * @param message
	 *            The body of the error message. The error code will be read
	 *            from <code>message[0],message[1]</code>.
	 */
	...
	/**
	 * Sets the state of the connection. Prints the new state on
	 * <code>System.err</code> and then awakes all threads waiting on this
	 * object's monitor.
	 * 
	 * @param newState
	 *            The state to set the connection into.
	 */
	 ...
	 * 
	 * @param targetState
	 *            The state bits that must be set for this method to return.
	 */
	 ...
	 * @throws UnknownTokenException
	 *             if a token encountered in the data stream is not known by the
	...
	String[] convert(byte[] bits) {
		...
		for (int i = 0; i < bits.length; i += 2) {
			...
			if (bits[i] == Token.TEXT) {
				 ...
				 * therefore we close the buffer and add it to the list here.
				 */
				if (inString) {
					str.append('\'');
				...
				try {
					...
					if ((list.size() > 0)
						...
						System.err.println();
						// Send a HUH message to the server. Note that the
						// official server
						// does not very well handle having a HUH message sent
						// to it.
						...
						try {
							...
							System.arraycopy(bits, i, errMessage, i + 6,
									bits.length - i);
							PAR = Token.convert(")");
	...
	/**
	 * Converts a message from its <code>String[]</code> token representation.
	 * 
	 * @param tokens
	 *            The mnemonic representation of the tokens
	 * @return the protocol representation of the tokens
	 * @throws UnknownTokenException
	 *             if a token encountered in the array is not known by the
	 *             <code>Token</code> singleton.
	 */
	...
	/**
	 * The consumer of incoming messages. This thread will run while the
	 * connection's state is not an error state.
	 * 
	 */
	public class Consumer extends Thread {
		...
		/**
		 * Creates a consumer. The specified <code>InputStream</code> will be
		 * used for reading incoming messages until the <code>Connection</code>
		...
		/**
		 * Continously read messages from the <code>InputStream</code> until the
		 * <code>Connection</code> enters an error state. When a complete
		 * message has been read, it will be passed on to
		 * <code>handleMessage</code>. If an I/O error occurs, the JVM will
		 * exit.
		 * 
		...
		public void run() {
			...
			while ((Connection.this.state & ERROR) == 0) {
				try {
					...
					while (input.read(msgType) == 0)
						; // Read the message type
						// System.out.println("Got message of type " +
						// msgType[0]);
					while (input.skip(1) == 0)
						; // Skip the pad
					while (input.read(length_hi) == 0)
						; // High octet of length
					while (input.read(length_lo) == 0)
						; // Low octet of length
